...
public function getById($id) {
    try {
        $query = "SELECT * FROM MikiCarros WHERE id = :id";
        $stmt = $this->conn->prepare($query);
...
public function create() {
    try {
        $query = "INSERT INTO MikiCarros (marca, detalhes, foto) VALUES (:marca, :detalhes, :foto)";
        $stmt = $this->conn->prepare($query);
...
public function update() {
    try {
        ...
        $query = "UPDATE MikiCarros SET marca = :marca, detalhes = :detalhes, foto = :foto WHERE id = :id";
        $stmt = $this->conn->prepare($query);
...
public function delete($id) {
    try {
        $query = "DELETE FROM MikiCarros WHERE id = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':id', $id, \PDO::PARAM_INT);
